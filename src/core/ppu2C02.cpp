...
{
    // 1. Mapper (Kartuş) tarama satırı bilgisini güncellesin (MMC3 vb. için)
    if (mask.render_background || mask.render_sprite) 
        cart->ppuScanline();

    // 2. Hazırlanan pikselleri display_buffer'a (paket servis tepsisi) kopyala
    // SCANLINE_SIZE genellikle 256'dır.
    memcpy(ptr_display + (scanline_counter * SCANLINE_SIZE), ptr_buffer, SCANLINE_SIZE * sizeof(uint16_t));
    
    scanline_counter++;

    // 3. Paket dolduğunda (8 satır olduğunda) Bus üzerinden ekrana gönder
    if (scanline_counter >= SCANLINES_PER_BUFFER) 
    { 
        // Burada Bus sınıfına "al bu 8 satırı ekrana bas" diyoruz
